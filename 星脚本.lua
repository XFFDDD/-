...
_G.Window = WindUI:CreateWindow({
    Title = gradient("WX脚本", Color3.fromHex("#FFFFFF"), Color3.fromHex("#00FF00")),
    Icon = "rbxassetid://81944629903864",
    IconTransparency = 5,
    IconThemed = true,
    Author = gradient("欢迎您使用", Color3.fromHex("#00FFAA"), Color3.fromHex("#00AAFF")),
    Folder = "CloudHub",
...
========================================================
  桥接层 (Shim)
  - 让旧脚本里所有 `about:Toggle / Slider / Dropdown / Textbox / Button / Label / Keybind`
    以及 `XIAOPI:Tab(...)` / `PIJIAOBEN:section(...)` 的调用，
    自动映射到新 WindUI 的真实控件，从而完整保留全部功能逻辑。
========================================================
]]

local function makeSection(winduiTab, sectionName)
    -- WindUI 用 Divider + Paragraph 作为分区标题；返回控件容器
    if sectionName and sectionName ~= "" then
        pcall(function() winduiTab:Paragraph({ Title = sectionName, Desc = "" }) end)
    end

    local container = {}

    -- Toggle(title, flag, default, callback)
    container.Toggle = function(_, title, flag, default, callback)
        local val = (default == true)
        local ok, el = pcall(function()
            return winduiTab:Toggle({
                Title = title or "开关",
                Desc = flag or "",
                Value = val,
                Callback = function(v) if callback then callback(v) end end,
            })
        end)
        return el
    end

    -- Slider(title, flag, default, min, max, _, callback)
    container.Slider = function(_, title, flag, default, min, max, _, callback)
        local d, lo, hi = tonumber(default) or 50, tonumber(min) or 0, tonumber(max) or 100
        if lo > d then lo = d end
        if hi < d then hi = d + 1 end
        local step = 1
        if hi - lo > 1000 then step = math.ceil((hi - lo) / 500) end
        local ok, el = pcall(function()
            return winduiTab:Slider({
                Title = title or "滑块",
                Desc = flag or "",
                Value = { Min = lo, Max = hi, Default = d },
                Step = step,
                Callback = function(v) if callback then callback(v) end end,
            })
        end)
    ...
    -- Dropdown(title, flag, {options}, callback)
    container.Dropdown = function(_, title, flag, options, callback)
        local opts = options or {}
        if type(opts) ~= "table" then opts = { tostring(opts) } end
        local first = opts[1]
        local ok, el = pcall(function()
            return winduiTab:Dropdown({
                Title = title or "下拉列表",
                Desc = flag or "",
                Values = opts,
                Value = first,
    ...
    -- Textbox(title, flag, placeholder, callback)
    container.Textbox = function(_, title, flag, placeholder, callback)
        local ok, el = pcall(function()
            return winduiTab:Input({
                Title = title or "输入框",
                Desc = flag or "",
                Value = "",
                Placeholder = placeholder or "请输入内容",
                Callback = function(v) if callback then callback(v) end end,
            })
        end)
        return el
    ...
    container.Button = function(_, title, callback)
        local ok, el = pcall(function()
            return winduiTab:Button({
                Title = title or "按钮",
    ...
    -- Label(text) -> 返回带 .Text 可写的对象（兼容旧逻辑 labels.xxx.Text = ...）
    container.Label = function(_, text)
        local mock = { Text = text or "" }
        local ok, el = pcall(function()
            return winduiTab:Paragraph({ Title = text or "", Desc = "" })
        end)
        if ok and el then
            -- 代理：对 el.Title / el:SetTitle 的写入同步到 mock.Text
            return setmetatable(mock, {
                __newindex = function(_, k, v)
                    mock[k] = v
                    if k == "Text" then
                        pcall(function() el:SetTitle(v) end)
                    end
                end,
                __index = function(_, k)
                    if k == "Text" then return mock.Text end
                    return el[k]
                end,
            })
        end
    ...
    -- Keybind(title, keyCode, callback)
    container.Keybind = function(_, title, keyCode, callback)
        -- WindUI 用 Bind；降级为 Button
        pcall(function()
            winduiTab:Button({
                Title = title or "快捷键",
                Desc = tostring(keyCode),
                Callback = function() if callback then callback(true) end end,
            })
        end)
    end
...
local function makeTab(winduiWindow, tabName, icon)
    local wt = winduiWindow:Tab({ Title = tabName, Icon = icon or "layout-grid" })
    local tabObj = { _windui = wt }

    -- section(name, _) -> 控件容器
    tabObj.section = function(_, name, _)
        return makeSection(wt, name)
    end

    return tabObj
end

-- 兼容旧 UI 库 PSU:new(name)
local PSU = {}
PSU.__index = PSU
function PSU:new(name)
    return {
        -- Tab(name, icon)
        Tab = function(self, name, icon) return makeTab(_G.Window, name, icon) end,
    }
end

_G.PSU = PSU
-- 旧脚本: local XIAOPI = PSU:new("星脚本")
-- 用全局变量暴露，供下方原脚本使用
...
run(function()
    _G.ScriptLoaderConfig = {
        scripts = {
             ...
             ["在超市生活一周"] = 'getgenv().XiaoPi="皮脚本-在超市生活一周" loadstring(game:HttpGet("https://raw.githubusercontent.com/zilinskaslandon/XingJiaoBen-2026-/refs/heads/main/%E6%98%9F%E8%84%9A%E6%9C%AC-%E5%9C%A8%E8%B6%85%E5%B8%82%E7%94%9F%E6%B4%BB%E4%B8%80%E5%91%A8.lua"))()',
...
run(function()
    about:Dropdown("选择服务器", "ServerSelector", {"伐木大亨2", "在超市生活一周", "内脏与黑火药", "自然灾害", "成长王国", "提升模拟器", "驾驶帝国", "尺寸传奇模拟器", "河北唐县","种植花园"}, function(Value)
...
local function createPerfPatch()
    ...
    local function applyOptimizations()
        originalValues = {
            espUpdateRate = _G.ESPConfig and _G.ESPConfig.UpdateRate,
            motionBlur = _G.MotionBlurConfig and _G.MotionBlurConfig.enabled,
            bloom = _G.GraphicsState and _G.GraphicsState.bloomEnabled,
            dof = _G.GraphicsState and _G.GraphicsState.depthOfFieldEnabled,
            blur = _G.GraphicsState and _G.GraphicsState.blurEnabled,
            sayInterval = _G.ChatConfig and _G.ChatConfig.sayInterval,
            autoSayInterval = _G.ChatConfig and _G.ChatConfig.autoSayInterval,
            microStep = _G.FlyConfig and _G.FlyConfig.MICRO_STEP_INTERVAL,
            aimNPC = _G.AimBotState and _G.AimBotState.aimNPC
        }
        
        if _G.ESPConfig then _G.ESPConfig.UpdateRate = 5 end
        if _G.MotionBlurConfig then _G.MotionBlurConfig.enabled = false end
        if _G.GraphicsState then
            _G.GraphicsState.bloomEnabled = false
            _G.GraphicsState.depthOfFieldEnabled = false
            _G.GraphicsState.blurEnabled = false
        end
        if _G.ChatConfig then
            _G.ChatConfig.sayInterval = 1.0
            _G.ChatConfig.autoSayInterval = 1.0
        end
        if _G.FlyConfig then
            _G.FlyConfig.MICRO_STEP_INTERVAL = 0.05
        end
        if _G.AimBotState then
    ...
    local function restoreSettings()
        if originalValues.espUpdateRate then _G.ESPConfig.UpdateRate = originalValues.espUpdateRate end
        if originalValues.motionBlur ~= nil then _G.MotionBlurConfig.enabled = originalValues.motionBlur end
        if originalValues.bloom ~= nil then _G.GraphicsState.bloomEnabled = originalValues.bloom end
        if originalValues.dof ~= nil then _G.GraphicsState.depthOfFieldEnabled = originalValues.dof end
        if originalValues.blur ~= nil then _G.GraphicsState.blurEnabled = originalValues.blur end
        if originalValues.sayInterval then _G.ChatConfig.sayInterval = originalValues.sayInterval end
