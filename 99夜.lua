...
local WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/xiaoxi9008/-UI/refs/heads/main/Wind.lua"))()
...
Window:EditOpenButton(
    {
        Title = "<font color='#FFFFFF'>X</font><font color='#CCCCCC'>I</font><font color='#999999'>A</font><font color='#666666'>O</font><font color='#444444'>X</font><font color='#333333'>I</font> <font color='#666666'>H</font><font color='#444444'>U</font><font color='#222222'>B</font>99个森林夜",
        CornerRadius = UDim.new(0, 13),
        StrokeThickness = 1.5,
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 0, 0)),        -- 黑色
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 255, 255)), -- 白色
            ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 0))         -- 黑色
        }),
        Draggable = true
    }
)
...
-- 黑白流动边框函数
local function createBlackWhiteBorder(window, speed)
    local mainFrame = window.UIElements.Main
    if not mainFrame then return nil end
    
    local existingStroke = mainFrame:FindFirstChild("BlackWhiteStroke")
    ...
    local bwStroke = Instance.new("UIStroke")
    bwStroke.Name = "BlackWhiteStroke"
    bwStroke.Thickness = 2
    bwStroke.Color = Color3.new(1, 1, 1)
    bwStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    bwStroke.LineJoinMode = Enum.LineJoinMode.Round
    bwStroke.Parent = mainFrame
    
    -- 创建黑白渐变色
    local gradient = Instance.new("UIGradient")
    gradient.Name = "BWEffect"
    
    -- 黑白两色交替的关键点
    -- 创建多个黑白交替的色条，实现流动感
    gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0.00, Color3.fromRGB(0, 0, 0)),      -- 黑
        ColorSequenceKeypoint.new(0.25, Color3.fromRGB(255, 255, 255)), -- 白
        ColorSequenceKeypoint.new(0.50, Color3.fromRGB(0, 0, 0)),      -- 黑
        ColorSequenceKeypoint.new(0.75, Color3.fromRGB(255, 255, 255)), -- 白
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(0, 0, 0))       -- 黑
    })
    
    gradient.Rotation = 0
    gradient.Parent = bwStroke
    
    return bwStroke
end

local function startBlackWhiteAnimation(window, speed)
    local mainFrame = window.UIElements.Main
    if not mainFrame then return nil end
    
    local bwStroke = mainFrame:FindFirstChild("BlackWhiteStroke")
    if not bwStroke then return nil end
    
    local gradient = bwStroke:FindFirstChild("BWEffect")
    if not gradient then return nil end
    
    local animation = game:GetService("RunService").Heartbeat:Connect(function()
        if not bwStroke or bwStroke.Parent == nil then
        ...
        local time = tick()
        -- 让渐变角度持续旋转，实现黑白流动效果
        gradient.Rotation = (time * speed * 30) % 360
    end)
...
-- 创建黑白边框并启动动画
local bwStroke = createBlackWhiteBorder(Window, currentSpeed)
if bwStroke then
    borderAnimation = startBlackWhiteAnimation(Window, currentSpeed)
end
