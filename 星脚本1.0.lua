...
local UI = {}
UI.__sections = {}   -- name -> section handle

-- 追踪当前正在填充的 section，供 Create 方法把元素归入对应容器
local _currentSection = nil

local function snapshot(page)
    local set = {}
    for _, c in ipairs(page:GetChildren()) do set[c] = true end
    return set
end
local function diff(page, oldSet)
    local list = {}
    for _, c in ipairs(page:GetChildren()) do
        if not oldSet[c] and c:IsA("GuiObject") then
            list[#list + 1] = c
        end
    end
    return list
end
local function reparentToCurrent(page, oldSet)
    if not _currentSection or not _currentSection._container then return end
    local news = diff(page, oldSet)
    for _, obj in ipairs(news) do
        obj.Parent = _currentSection._container
    end
end

--[[
  新版 black ui 库模型:
    window:CreateTab(name)       -> 左侧导航栏(全脚本只一次)
    tab:CreateFrame(pageName)    -> 内容 ScrollingFrame(page) + 一个 PageButton
--]]
local _mainTab = nil
...
function UI:CreateTab(window, name, icon)
    ...
    page.Visible = false
    local tabObj = { Window = window, Tab = Tab, _page = page, _els = {}, _sections = {} }
    setmetatable(tabObj, { __index = UI })
    return tabObj
end

-- section: 现在【真实可折叠】
--   defaultOpen: true 初始展开(默认), false 初始折叠
function UI:section(name, defaultOpen)
    defaultOpen = (defaultOpen ~= false)
    local page = self._page

    -- 1) 容器 Frame：装该 section 的所有子元素
    local container = Instance.new("Frame")
    container.Name = "SectionContainer_" .. name
    ...
    -- 2) 标题栏(用库的 CreateToggle, 真实可点击)
    --    state: true=展开, false=折叠. 直接用它驱动容器显隐, 状态永远同步.
    local snap = snapshot(page)
    local titleToggle = page:CreateToggle("▼ " .. name, "", function(state)
        container.Visible = (state == true)
        pcall(function()
            titleToggle:UpdateToggle((container.Visible and "▼ " .. name) or ("▲ " .. name))
        end)
    end)
    -- 标题栏自身留在 page 顶层, 不随折叠隐藏
    local titleFrames = diff(page, snap)
    for _, obj in ipairs(titleFrames) do
        obj.Parent = page
    end
    ...
    local handle = {
        ...
        SetOpen = function(_, open)
            container.Visible = not not open
            pcall(function()
                titleToggle:UpdateToggle((container.Visible and "▼ " .. name) or ("▲ " .. name))
            end)
        end,
        IsOpen = function() return container.Visible end,
    }
    UI.__sections[name] = handle
    setmetatable(handle, { __index = UI })

    _currentSection = handle
    if not defaultOpen then
        pcall(function() titleToggle:UpdateToggle("▲ " .. name) end)
    end
    return handle
end
...
function UI:Label(text)
    local page = self._page
    local snap = snapshot(page)
    local el = page:CreateLabel(text)
    reparentToCurrent(page, snap)
    self._els[text] = el
    return el
end

-- Button  (Button(name, [desc,] callback))
function UI:Button(name, a, b)
    ...
    local page = self._page
    local snap = snapshot(page)
    local el = page:CreateButton(name, desc or "", cb)
    reparentToCurrent(page, snap)
    return el
end

-- Toggle (Toggle(name, flag, default, callback))
function UI:Toggle(name, flag, default, cb)
    ...
    local page = self._page
    local snap = snapshot(page)
    local tgl = page:CreateToggle(name, "", function(v) cb(v) end)
    reparentToCurrent(page, snap)
    return tgl
end

-- Slider (Slider(title, flag, def, min, max, prec, callback))
function UI:Slider(name, flag, def, min, max, prec, cb)
    cb = cb or function() end
    local page = self._page
    local snap = snapshot(page)
    local sld = page:CreateSlider(name, min or 0, max or 100, function(v) cb(v) end)
    reparentToCurrent(page, snap)
    return sld
end

-- Textbox (Textbox(name, flag, placeholder, callback))
function UI:Textbox(name, flag, placeholder, cb)
    cb = cb or function() end
    local page = self._page
    local snap = snapshot(page)
    local el = page:CreateBox(placeholder or name, "Submit", function(v) cb(v) end)
    reparentToCurrent(page, snap)
    return el
end
...
function UI:Bind(name, defaultKey, cb)
    ...
    local page = self._page
    local snap = snapshot(page)
    local el = page:CreateBind(name, tostring(defaultKey or "Unknown"), function(v) cb(v) end)
    reparentToCurrent(page, snap)
    return el
end
UI.Keybind = UI.Bind

-- Dropdown (Button + 当前值 Label 模拟)
function UI:Dropdown(name, flag, options, cb)
    cb = cb or function() end
    local page = self._page
    local snap = snapshot(page)
    page:CreateLabel("▼ " .. name)
    local opts = (type(options) == "table") and options or {}
    local current = opts[1]
    local valLabel = page:CreateLabel("当前: " .. tostring(current or "未选择"))
    local function upd()
        pcall(function() valLabel:UpdateLabel("当前: " .. tostring(current)) end)
    end
    for _, opt in ipairs(opts) do
        page:CreateButton(tostring(opt), "", function()
            current = opt
            upd()
            cb(opt)
        end)
...
function UI:ColorPicker(name, flag, default, cb)
    ...
    local page = self._page
    local snap = snapshot(page)
    local el = page:CreateColorPicker(name, function(col) cb(col) end)
    reparentToCurrent(page, snap)
    return el
end
...
local PIJIAOBEN = UI:CreateTab(Window, "旋转与范围", "136169594232359")
...
local PIJIAOBEN = UI:CreateTab(Window, "传送与甩飞", "136169594232359")
...
run(function()
    ...
    local PIJIAOBEN = UI:CreateTab(Window, "自动说话", "136169594232359")
...
local PIJIAOBEN = UI:CreateTab(Window, "时间", "136169594232359")
...
local PIJIAOBEN = UI:CreateTab(Window, "透视", "136169594232359")
...
run(function()
    ...
    local function createESP(character, player)
        ...
        nameLabel.Text = "星脚本用户[" .. player.Name .. "]"
...
run(function()
    ...
    about:Toggle("星脚本用户透视", "", false, function(state)
        if state then
            module.enable()
        else
            module.disable()
...
local PIJIAOBEN = UI:CreateTab(Window, "自瞄", "136169594232359")
...
local PIJIAOBEN = UI:CreateTab(Window, "FE", "136169594232359")
...
about:Button("修改星脚本贴图天空", function()
...
local PIJIAOBEN = UI:CreateTab(Window, "音乐", "136169594232359")
...
local PIJIAOBEN = UI:CreateTab(Window, "画质光影", "136169594232359")
...
local acsTab = UI:CreateTab(Window, "ACS漏洞", "136169594232359")
...
local PIJIAOBEN = UI:CreateTab(Window, "飞行与飞车", "136169594232359")
